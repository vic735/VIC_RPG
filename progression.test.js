...
test('主動型追擊、警戒型近距離觸發、膽怯型逃跑', () => { for (const [type, level, expected] of [['goblin', 1, 1], ['wolf', 1, 1], ['timid', 5, -1]]) { const r = make(); r.level = level; r.position = { x: 500, y: 1000 }; const e = { id: 'test', mapId:r.currentMapId, type, level: 1, x: 570, y: 1000, homeX: 570, homeY: 1000, defeatedUntil: 0 }; r.world.enemies = [e]; W.update(r, .1, { x: 0, y: 0 }); assert.equal(Math.sign(570 - e.x), expected); } });
test('邊界、探索發現、小地圖標記與區域難度', () => { const r = make(); r.currentMapId=D.dungeons[0].mapId; r.position = { x: D.dungeons[0].x, y: D.dungeons[0].y + 65 }; W.update(r, .1, { x: 0, y: 0 }); assert.ok(r.world.discoveredDungeons.includes('abandoned_mine')); assert.equal(W.nearby(r).kind, 'dungeon'); assert.equal(W.blocked(-1, 300), true); assert.ok(P.regionLevel(1200, 1050) > P.regionLevel(100, 100)); });
