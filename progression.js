(function (root) {
  ...
  function enemyDefinition(type,level){const source=D.monsters[type];if(!source)throw Error('未知敵人');const stats={};for(const [k,v]of Object.entries(source.stats))stats[k]=source.growth?(k==='agility'||k==='luck'?v+(level-1)*source.growth[k]:v*(1+(level-1)*source.growth[k])):v*(1+(level-1)*D.balance.enemyGrowth);return {...source,stats,level};}
  function battleFor(run, encounter, rng = Math.random) {
    ...
    const b = new C.Battle({ rng, balance50:true, preserveResources:!!run.dungeon, stats: statsFor(run, false), build: run.build, moveLevels: run.moveLevels, moveScale, enemy: enemyDefinition(encounter.type, encounter.level), equipmentEffects: effects, rules: {
