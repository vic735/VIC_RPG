...
(function(root){function enrich(D){
 ...
 for(const [ri,r]of D.world.regions.entries()){
  designs[ri].forEach(([sprite,name,elements,moves],i)=>{const id=r.id+'_'+i,base=clone(D.monsters[sprite]);D.monsters[id]={...base,id,name,sprite,elements,moves,behavior:['neutral','active','guard','timid'][i],radius:[0,225,140,185][i],speed:[30,100,75,80][i],stats:{...base.stats,mana:Math.max(base.stats.mana,45+ri*8),agility:base.stats.agility+(ri===3?38:ri*4)},skills:ri===0?[]:ri===1?['chill']:ri===2?['heavy_master']:ri===3?['fast_cast']:['night_erosion'],expFactor:1+ri*.05,dropChance:.25,dropPool:r.id+'_field',growth:{hp:.045,stamina:.016,mana:.016,agility:.18,luck:.07},color:['#a3b481','#70b1a0','#d89966','#a0cede','#b3a0d1'][ri]};r.enemyPools.push(id);});
  const elite=r.id+'_elite';D.monsters[elite]={...clone(D.monsters[r.enemyPools[1]]),id:elite,name:r.name+'菁英',sprite:'boss',elite:true,behavior:'guard',radius:170,stats:{hp:350,stamina:75,mana:65,agility:35+(ri===3?30:0),luck:9},moves:[...designs[ri][1][3],'crush'],skills:['unyielding'],expFactor:1.8,dropChance:.75};r.elitePools.push(elite);
 ...
 for(const [i,r]of D.world.regions.entries())D.rewardPools[r.id+'_field'].entries.push({rewardType:'talents',rewardIds:fieldSkills[i],weight:2,rarity:'common',requirements:{}});
 ...
 D.rewardPools.greywind_field.entries.push({rewardType:'moves',rewardIds:['quick','fire','heavy','interrupt'],weight:3,rarity:'common',requirements:{}});
